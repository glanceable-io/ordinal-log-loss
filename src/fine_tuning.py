        ...
        #list(datasets.keys()):
            ...
            for k in tqdm([1,2,3,4,5]):
                ...
                training_args = TrainingArguments(
                    output_dir=f"{Path.home()}/glanceable-research/src/research/ordinal_loss_classification/output_models/{data_file}/training/{model_name}/",          # output directory
                    fp16=True,
                    ...
                    logging_dir=f"{Path.home()}/glanceable-research/src/research/ordinal_loss_classification/output_models/{data_file}/logs/{model_name}_{epochs_}_ep_{learning_rate_}_lr_{train_batch_size_}_batch",            # directory for storing logs
                )
                if loss_type == "CE":
                    trainer = Trainer(
                        ...
                        callbacks=[EarlyStoppingCallback(early_stopping_patience=5)]         # evaluation dataset
                    )
                ...
                trainer.save_model(f"{Path.home()}/glanceable-research/src/research/ordinal_loss_classification/output_models/{data_file}/saved_models/{model_name}_{epochs_}_ep_{learning_rate_}_lr_{train_batch_size_}_batch")
                ...
                requests.post(webhook_url, json.dumps(dump))
                output_path_metrics = f'{Path.home()}/glanceable-research/src/research/ordinal_loss_classification/output_metrics/fine_tuning_metrics.csv'
